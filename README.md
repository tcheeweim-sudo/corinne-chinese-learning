...
Practice 听写 supports current and archived sets, Study or Test, and 3, 5, or all items. Study shows the curated answer data and uses guided Hanzi Writer. Test plays audio and uses a blank pointer-event canvas for finger or S Pen writing. It reveals the answer only after **Check answer**, then records the child's immutable first self-check separately from eventual completion. It does not perform OCR or automated handwriting grading.

Revision offers Quick (5), Full (12 by default, adjustable from 10–15), and Weak Words. Parent settings control lesson range, recognition/writing mix, current and archived 听写 inclusion, session size, and weak-item emphasis. Weak Words never broadens the configured scope when empty.
